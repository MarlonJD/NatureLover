        ...
        data: function(){
            return{
                ...
                gridData : [
                    ...
                    {name : '<?php echo $row->name; ?>',shortDetail: "<?php echo $row->shortDetail; ?>", place : '<?php echo $row->place; ?>',date : '<?php echo date_tr('j F Y, l', $row->date); ?>',date2 : '<?php if (!($row->date2 == "0000-00-00")) { echo "- ",date_tr('j F Y, l', $row->date2); } ?>',udate: '<?php echo date_tr('j F Y', $row->date); ?>', type: '<?php getTypeNameByID($row->type); ?>',subType: '<?php getSubTypeNameByID($row->subType); ?>' , action : 'edit', numberOfP : ''},
                    <?php } ?>
                ]};
        },
